...
def send_verification_email(email: str, verification_link: str) -> None:
    """Send a verification email using SMTP SSL (Port 465) for cloud compatibility."""
    ...
    sender_address = SMTP_FROM_EMAIL if "@" in SMTP_FROM_EMAIL and "localhost" not in SMTP_FROM_EMAIL else clean_username
    message["From"] = f"LogTriage AI <{sender_address}>"
    message["To"] = email
...
def signup(user_in: UserSignup, db: Session = Depends(get_db)):
    ...
    try:
        ...
        print(f"[WARNING] [Email Verification] Failed to send email to {user.email}: {e}")
